...
```
### 3. Update the agent service 

### 4. Update the API service

### 5. Add tests for the agent


Once the UI is complete, you can move onto updating the API 


## Add API Endpoints for the agent

First, add a new endpoint for the agent in the `infra/api.ts` file. 
...
```
Then add a new queue for the agent in the `infra/queues.ts` file. 
...
Update the `orderTopic` to be linked with the new queue in the `infra/topic.ts` file. 
...
Here you will wire up the orchestrator so it publishes an order to the `orderTopic` and then that will trigger the agent queue. 

#### Primary Adapter

The primary adapter handles incoming API Gateway events and validates user input:

Start with the `request-your-agent.adapter.ts` file. This will be in the `adapters/primary` folder of the orchestrator. 
```typescript
...
Next update the metadata layer to add your agent inputs and outputs for the orchestrator `agent-plane.schema.ts` file. 

```typescript
// packages/core/src/orchestrator/metadata/order.schema.ts

export const YourAgentOrderInputSchema = BasePayloadSchema.extend({
  field1: z.string(),
  field2: z.string(),
  deliverableName: z.string(),
  agentId: z.string()
...
export const YourAgentOrderOutputSchema = z.object({
  orderId: z.string(),
...
export type YourAgentOrderInput = z.infer<typeof YourAgentOrderInputSchema>;
export type YourAgentOrderOutput = z.infer<typeof YourAgentOrderOutputSchema>;
```

Then update the `http-responses.schema.ts` file to add your agent response that will be returned to the client.

This should be a success response that will be returned from the orchestrator. 

The process will occur in the background and the client will receive a 201 response.
...
export const OrchestratorHttpResponses = {
  ...HttpResponses,
  YourAgentRequestReceived: (params: HttpResponseParams<OrderResponseBody>) => 
    createHttpResponse(HttpStatusCode.CREATED, params),
}
```

Also update the `order.enum.ts` to define the new agent cost. 
...
In this usecase you will create the order and publish it to the `orderTopic`. 
...
```
Finally you will need to wire up the functions in '/functions/src'

Update the `orchestrator.api.ts` file to include your new agent handler. 

```typescript
// packages/functions/src/orchestrator.api.ts
export const requestYourAgentHandler = createHandler(requestYourAgentAdapter);
```

### 5. Implement Agent Plane Logic

The agent plane implementation follows the [AMU](/guides/amu-pattern) pattern for clean architecture and separation of concerns:

Here you will implement the SQS handler for the agent, it's main business logic. 

Create a new directory for your agent in the `agent-plane` folder. 

```bash
cd api 
mkdir -p packages/core/src/agent-plane/your-agent
```

Within the new directory you will create the following files: 
...
- `usecases/your-agent-action.usecase.ts`
```
...
#### Use Case
Inside the `usecases` folder you will create the main business logic for your agent. 
Below is an example of the `your-agent-action.usecase.ts` file. 
...
The secondary adapters are used to interact with external services. 

In this case we are using OpenAI to generate the deliverable using the Assitants API. 
...
Update the `agent-plane.api.ts` file to include your new agent handler. 
```typescript
// packages/functions/src/agent-plane.api.ts
export const yourAgentHandler = createSQSHandler(createAgentAdapter);
```
